...
Font::Font(std::wstring fontName, int fontSize, D3DCOLOR fontColor)
{
	...
	HRESULT hr = D3DXCreateFont(GameSystem::GetInstance()->GetDeviceDX(), fontSize, 0, FW_BOLD, 1, false, DEFAULT_CHARSET,
...
{
	D3DXMATRIX matrix;
	D3DXMatrixTransformation2D(&matrix, NULL, 0.0f, NULL, NULL,
		0.0f, NULL);
...
{
	ID3DXFont* testFontDX;
	HRESULT hr = D3DXCreateFont(GameSystem::GetInstance()->GetDeviceDX(), _fontsize, 0, FW_NORMAL, 1, false, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
